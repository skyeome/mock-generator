...
describe("AIClient", () => {
  ...
  describe("analyzeSchema", () => {
    ...
    it("should return null when AI is disabled", async () => {
      ...
      const result = await client.analyzeSchema(
        sampleSchema,
    ...
    it("should handle complex nested schema", async () => {
      ...
      const nestedResponse: AISchemaAnalysis = {
        ...
        fieldHints: [
          {
            fieldPath: "user.name",
            suggestedSemantic: "fullName",
            confidence: 0.8,
          },
          {
            fieldPath: "user.address.city",
            suggestedSemantic: "city",
            confidence: 0.9,
          },
          {
            fieldPath: "user.address.country",
            suggestedSemantic: "country",
            confidence: 0.9,
          },
        ],
    ...
    it("should handle schema with multiple semantic types", async () => {
      ...
      const ecommerceResponse: AISchemaAnalysis = {
        ...
        fieldHints: [
          ...
          {
            fieldPath: "sellerName",
            suggestedSemantic: "company",
            confidence: 0.85,
          },
          {
            fieldPath: "category",
          ...
          { fieldPath: "stock", suggestedSemantic: "integer", confidence: 0.9 },
        ],
        coherenceGroups: [
          ["price", "currency"],
          ["sellerId", "sellerName"],
      ...
      const mockAI = createMockAI({
        response: JSON.stringify(ecommerceResponse),
      });
    ...
    it("should handle response with optional relatedFields", async () => {
      ...
      const mockAI = createMockAI({
        response: JSON.stringify(responseWithRelated),
      });
    ...
    it("should handle response with optional reasoning field", async () => {
      ...
      const mockAI = createMockAI({
        response: JSON.stringify(responseWithReasoning),
      });
      ...
      expect(result?.fieldHints[0].reasoning).toBe(
        "Field name and format suggest email address",
      );
    });
    ...
    it("should handle primitive type schemas", async () => {
      ...
      const mockAI = createMockAI({
        response: JSON.stringify(primitiveResponse),
      });
    ...
    it("should handle confidence values correctly", async () => {
      ...
      const mockAI = createMockAI({
        response: JSON.stringify(responseWithVaryingConfidence),
      });
...
describe("AIClient - Response Parsing Edge Cases", () => {
  ...
  it("should handle AI client internal error during parsing", async () => {
    ...
    const mockAI = createMockAI({
      response: "{ broken json without proper formatting",
    });
...
describe("AIClient - Cloudflare Response Edge Cases", () => {
  ...
  const validResponse: AISchemaAnalysis = {
    domainContext: "test",
    fieldHints: [
      { fieldPath: "name", suggestedSemantic: "fullName", confidence: 0.9 },
    ],
    coherenceGroups: [],
  };
...
describe("AIClient - Parse Response Edge Cases", () => {
  ...
  it("should handle malformed JSON in response", async () => {
    const client = new AIClient();
    const malformedJSON =
      '{ "domainContext": "test", "fieldHints": [missing closing bracket';
